...
public class UserServiceImpl extends BaseServiceImpl<UserEntity, UserQuery, Long> implements IUserService {
	...
	public Message save(UserBean userBean) throws Exception {
		...
		//持久化后消息处理
		if(affectedConut==1L){
			return Message.getMessage(MessageTypeEnum.success,MessageAlias.USER_ADD_SUCCESS);
		}else{
			return Message.getMessage(MessageTypeEnum.failure,MessageAlias.USER_ADD_FAILURE);
		}
	}
